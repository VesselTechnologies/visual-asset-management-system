    ...
    const showDownloadConfirmation = async (version: AssetVersion) => {
        ...
        try {
            ...
            if (success && response) {
                ...
                const downloadableFiles = response.files.filter(
                    (file: FileVersion) => !file.isPermanentlyDeleted
                );

                // Calculate total size
                const totalSize = downloadableFiles.reduce(
                    (sum: number, file: FileVersion) => sum + (file.size || 0),
                    0
                );
                    ...
                    <SpaceBetween direction="horizontal" size="xs">
                        <Button
                            ...
                            onClick={(e) => {
                                ...
                                console.log(
                                    "AssetVersionList - Select button clicked for version:",
                                    item
                                );
                                console.log(
                                    "AssetVersionList - Current versionsForComparison before selection:",
                                    versionsForComparison
                                );
                                ...
                                console.log(
                                    "AssetVersionList - Selection completed for version:",
                                    item.Version
                                );
                            }}
                        ...
                        <Button
                            ...
                            onClick={(e) => {
                                e.stopPropagation(); // Prevent row selection
                                console.log(
                                    "AssetVersionList - Compare with Current button clicked for version:",
                                    item
                                );
            ...
            <Table
                ...
                onSelectionChange={({ detail }) => {
                    ...
                    if (detail.selectedItems.length === 0 && selectedVersion) {
                        console.log(
                            "AssetVersionList - Preventing selection clearing, keeping current selection"
                        );
                    ...
                    if (detail.selectedItems.length > 0) {
                        ...
                        if (
                            !selectedVersion ||
                            selectedVersion.Version !== detail.selectedItems[0].Version
                        ) {
                            handleSelectionChange(detail.selectedItems);
                        } else {
                            console.log(
                                "AssetVersionList - Ignoring selection of already selected version"
                            );
                        }
                    }
                }}
                    ...
                    {downloadLoading ? (
                        ...
                        <>
                            ...
                            {downloadDetails.fileCount > 0 ? (
                                <Box>
                                    <p>
                                        You will be redirected to the download page where you can
                                        select a folder to save these files. The files will maintain
                                        their relative paths within the selected folder.
                                    </p>
                                    <p>
                                        <strong>Note:</strong> Permanently deleted files are not
