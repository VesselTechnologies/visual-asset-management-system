...
export const AssetVersionManager: React.FC = () => {
    ...
    const [comparisonType, setComparisonType] = useState<"two-versions" | "with-current">(
        "two-versions"
    );
    ...
    const handleCompareSelect = (version: AssetVersion) => {
        ...
        if (compareMode) {
            if (versionToCompare) {
                ...
                console.log(
                    "AssetVersionManager - Already have versionToCompare, setting selectedVersion"
                );
            ...
            setSelectedVersion((currentSelectedVersion) => {
                ...
                if (
                    ...
                    currentSelectedVersion.Version === versionToSelect.Version
                ) {
    ...
    const handleVersionSelectionForComparison = (
        version: AssetVersion,
        forceWithCurrent = false
    ) => {
    ...
    const startComparison = () => {
        ...
        if (comparisonType === "two-versions") {
            if (versionsForComparison.length === 2) {
                console.log(
                    "Starting two-versions comparison with versions:",
                    versionsForComparison
                );
            ...
            if (versionsForComparison.length > 0) {
                console.log(
                    "Starting with-current comparison with version:",
                    versionsForComparison[0]
                );
                setVersionToCompare(versionsForComparison[0]);
                setCompareMode(true);
                setCompareWithCurrent(true);
        ...
        <AssetVersionContext.Provider value={contextValue}>
                    ...
                    <Header
                        ...
                        actions={
                            <SpaceBetween direction="horizontal" size="xs">
                                ...
                                {compareMode ? (
                                    <Button onClick={toggleCompareMode} variant="primary">
                                        Exit Compare Mode
                                    </Button>
                                ) : (
                                    <Button
            ...
            >
                <SpaceBetween direction="vertical" size="l">
                        ...
                        >
                            <SpaceBetween direction="vertical" size="l">
                                <SegmentedControl
                                    ...
                                    onChange={({ detail }) => {
                                        const newType = detail.selectedId as
                                            | "two-versions"
                                            | "with-current";
                                        console.log(
                                            "SegmentedControl onChange - new type:",
                                            newType
                                        );
                                        // Only reset selections if actually changing the type
                                        if (newType !== comparisonType) {
                                            console.log(
                                                "SegmentedControl onChange - changing type from",
                                                comparisonType,
                                                "to",
                                                newType
                                            );
                                    ...
                                    <div>
                                        <SpaceBetween direction="vertical" size="s">
                                            ...
                                            {versionsForComparison.length === 0 ? (
                                                <Box>
                                                    No versions selected. Select versions from the
                                                    list below.
                                                </Box>
                                            ) : (
                                    ...
                                    <div>
                                        <SpaceBetween direction="vertical" size="s">
                                            ...
                                            {comparisonType === "two-versions" ? (
                                                <Box>
                                                    <p>
                                                        Select two versions from the list below to
                                                        compare their files.
                                                    </p>
                                                ...
                                                <Box>
                                                    <p>
                                                        Select one version from the list below to
                                                        compare with the current files.
                                                    </p>
                                                    <p>
                                                        This will show what has changed since the
                                                        selected version, including:
                                                    </p>
                                                    <ul>
                                                        <li>
                                                            <span style={{ color: "#037f0c" }}>
                                            ...
                                            <Button
                                                ...
                                                disabled={
                                                    (comparisonType === "two-versions" &&
                                                        versionsForComparison.length !== 2) ||
                                                    (comparisonType === "with-current" &&
                                                        versionsForComparison.length === 0)
                                                }
                                            >
                                                Start Comparison
                    ...
                    {selectedVersion && !compareMode && <FileVersionsList />}
